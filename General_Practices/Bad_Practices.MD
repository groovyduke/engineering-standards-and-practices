...
I have helped to fix a lot of these bad practices over the years, I didn't get to fix all of them, Some were too ingrained and couldn't be
...
who I have learned a great deal. 

However, bad practice tend to seep in when a company is in a rush and developers are forced to cut corners. Like when a company is new in a
...
Every company/organization should make time to fix and cleanup bad practices rather than just kick the can down the road, The longer you
...
It is common to take the patterns you see in a codebase and perpetrate them for consistency, or with unexperienced developers, because they
don't know better yet. This will end up costing you time and money

On the other hand you shouldn't just go randomly fixing pieces of code, as you can step on other peoples toes, end up breaking other things
...
You think you've seen bad code? I've seen so much bad code that in a day of frustration I came up with chorus to a song about bad code:
```
Everybody welcome to the Shit Show!
You're gonna see some crazy Shit! at the Shit Show!
And nobody knows what to do about the Shit Show!
...
Then to go allow with it, I compiled a list of bad practices I've seen over the years. At one point I was thinking about feeding the 
...
Commented code should never be pushed up to a main version control repository. You look at this code and wonder is it important, why is
it here. Rather than waisting other peoples time just delete it, if it's in version control then you can always get it back or for the 
...
depending on the compiler. This is not to say that you need to make ever class super short, it is a balancing act. Depending on the language
...
places can be an exception to this where you may have a large switch statement and nothing else in the method, Although if you have a really
large switch statement that be a symptom of another bad practice.
...
you go past a few levels of this your code will be harder to read. Checkout this video about nesting code:
...
If you don't write tests for your code, either you or QA will be testing that code manually and sending it back to you will more bugs to 
...
could be run quicker and give us quicker feed back, but with more modern frameworks like Micronaut, integrations tests can be almost as fast
as unit test.
...
slightly different. In those cases it might make sense to either leave it alone or look to functional programing technics.
...
I've seen someone reimplementing `.contains` using a loop. Look at your SDK and google to see if there are libraries that solve you 
...
will be much better than if you implemented it your self.
### Tightly coupled code
A sign of tightly coupled code is when you fix one issue, and it breaks several other things, because that code was dependant ob the 
...
### Unused variables
Just delete unused variables, they only serve a purpose of asking why is this here, and just makes your code nosier than it needs to be.
...
In general, it is a good idea to keep the same name for something all the way from the UI back to the database. This will make is easier to
trace through the different levels of your application. In one place I worked  we had UI facing names for object, but in the backend DB
we had genetic algorithm name like gene, genotype, allele. so you had to remember what mapped to what if you wanted to look something up in
the DB.
### Acronyms
Technology in general used a lot of acronyms, try not to create your own, and don't use them when naming variables, this can make your code
more obsure and harder to read. Funny story one place I worked automatically made an acronym of a title for a unique identifier that was 
stored in an xml file, and one time I looked at the Annual New Users Security Metric and had a really good laugh.
### Magic numbers
It's always fun with you have magic numbers in code, being actual number or strings, because if you use that number or string in more than
...
I've known developers who refuse to learn anything new, they just want to do what they know, and anything new is a waste of time and an
inconveniences.
### Never upgrade your frameworks or libraries
This goes hand in hand without not wanting to learn anything new. The longer you go without updating your framework or libraries the harder
...
writing out the logic. Regexes are mainly write only code, meaning you're write it, forget what it means and then never be able to read it 
...
Having client specific code and an execute to have duplicate code, rather than having a base classes that you only extend and override if you
need to for a customer. Although better companies to work for will have somethings configurable, but have the same codebase for all clients.
...
Closure and functional programing can be very helpful and make certain classes of problems easier to read and maintain. However, they are
generally less performant that methods, so if you are not taking advantage of the functional programing aspects of closer use
methods instead.
...
however is a condition is always false you can remove that coed completely.
...
This was an issue I saw a lot of in Grails. DB logic everywhere. this lead to a lot nested DB logic where transactions could become a problem.
A better way it to have a separation from your business logic and your DB code. Grails added Data Services which work to that end. Spring 
...
While storeing expensive calculations in the DB or in a cache can be good for performance, storing inexpensive calculation and lead to a 
lot of extra maintenance without any real gain.
### Use NoSql as if it was SQL with joins
No SQL databases aren't SQL and are usually meant to be denormalized, so you don't have to do slow joins. Although I've also see it where
...
having the join, and even slower than denormalize there data structure to match how you should model NoSQL.
### Forget your frameworks I'll do it manually.
Why use command object with Grails rich validation system, where you can write a bunch of if thens and validate it all yourself? That's 
...
It doesn't take that long to set up and IDE and you gain a tool that allows you to easily debug your code. I've worked with some people
who would not use the IDE and ask for changes that would make the code easier for them to debug. In the JVM/SpringBoot/Grails/Micronaut 
world the best IDE we have is Intellij. It is not cheap for a commercial licence, but it is a very helpful tool when maintain a code base. 
### Use `printlns` to debug
`Printlns` are a "poor mans" debugger, even intellij has a community edition, and while it has fewer features then the ultimate edition it's 
...
in production, so you know where to debug in your code. Or if you are tying to debug something that happens at the end of a long list.
### Don't profile your code
If you're code is slow you should profile your code with a profiler to find out what is causing the slow down so that you can fix it.
Jprofiler is a good tool for this.
### Don't believe in QA you want engineers to take "ownership"
Don't get me wrong engineers should write automated unit/integration tests, however manual QA has a different mind set, of how do I break
...
do it, and if that's not there primary job they will do a worse job of it. Engineers don't make good manual QA of their own code, because
they too engrossed into it and can't see the forest through the trees.

[Section dedicated to QA](./../Management/QA.MD)
### Deploying wars, instead of jars in containers.
This is more of a not keeping up to date with current engineering trends and taking advantage of not just new technology but current 
...
long term. Once apps get to be too big, it's time to start thinking about breaking it up into smaller services, and address issues that 
...
You shouldn't try to unit test external systems. If you want to involved external systems in your testing, it's better to user an integration
